...
class PubsManagement:
    ...
    def show_by_pubs(self, model, pub_id):
        result = {}
        for pub in self.db.session.query(model).filter_by(pub_id=pub_id).all():
            result.update({pub.pub_id: {'pub_name': pub.pub_name}})
        return json.dumps(result)
    ...
    def add(self, pub_name, model):
        if bool(self.db.session.query(model).filter_by(pub_name=pub_name).scalar()):
            ...
            data = model(pub_name)
            self.db.session.query.add(data)
...
class CocktailsManagement:
    ...
    def show(self, model, model2, cocktail_name):
        ...
        for cocktail in self.db.session.query(model).filter_by(cocktail_name=cocktail_name).all():
            avg_rating = self.db.session.query(func.avg(model2.rating)).filter_by(cocktail.cocktail_id).first()
            ratings_quantity = self.db.session.query(func.avg(model2.ratings_quantity)).filter_by(cocktail.cocktail_id).first()
    ...
    def add(self, model, cocktail_name, pub_id):
        if self.db.session.query(model).filter_by(cocktail_name=cocktail_name, pub_id=pub_id).first().scalar():
            return False
        else:
